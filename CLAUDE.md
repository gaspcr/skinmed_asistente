...
- `FM_FOTOS_LAYOUT` — FileMaker layout for creating patient photo records (default: `SetFotosPaciente_dapi`)
...
- `tens` — Legacy only. Uploads patient photos to FileMaker via a one-time UUID token: FileMaker triggers `POST /internal/tens/solicitud-foto` (shared-secret auth) which messages the TENS phone with the token, and the TENS's photo reply is resolved back to the real patient record entirely inside FileMaker — no patient-identifying data ever travels over WhatsApp
